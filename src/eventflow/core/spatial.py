...
def assign_to_grid(
    ...
    lf = (
        ...
        .with_columns(
            [
                (
                    (pl.col("_row_idx") * pl.lit(n_cols)) + pl.col("_col_idx")
                ).alias("grid_id")
            ]
        )
...
def assign_to_zones(
    ...
    # Build simple zone lookup
    zone_geoms = [wkt.loads(val) for val in zones[zone_geom_col]]
    zone_ids = zones[zone_id_col].to_list()

    def find_zone(lon: float, lat: float) -> int | None:
        """Find first zone containing the point."""
        point = geometry.Point(lon, lat)
        for zid, geom in zip(zone_ids, zone_geoms):
